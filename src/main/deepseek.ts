import type { ChatRequest, TestResult, ToolCall, ToolResult, ToolContext } from '../shared/types'
...
import { callDeepSeekStream, sanitizeContent, type StreamHandlers, type SingleCallResult } from './sse-stream'
import { trimContext, truncateToolResult } from './context-compress'
...
export async function agentLoop(
  ...
  while (round < agentConfig.maxToolRounds) {
    ...
    if (round === 1) {
      trimContext(messages, agentConfig)
    }
    ...
    if (result.finishReason === 'tool_calls' && result.toolCalls.length > 0) {
      ...
      for (let i = 0; i < execResults.length; i++) {
        ...
        if (item.status === 'fulfilled') {
          ...
          messages.push({
            role: 'tool',
            content: sanitizeContent(truncateToolResult(toolResult.content, agentConfig)),
            tool_call_id: tc.id
          })
        } else {
